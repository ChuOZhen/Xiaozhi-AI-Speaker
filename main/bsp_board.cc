...
static esp_err_t bsp_i2s_init(uint32_t sample_rate, int channel_format, int bits_per_chan)
{
    ...
    // 【优化】配置 I2S 标准模式，严格按照INMP441规格
    i2s_std_config_t std_cfg = {
        .clk_cfg = {
            .sample_rate_hz = sample_rate,
            .clk_src = I2S_CLK_SRC_DEFAULT,
            .ext_clk_freq_hz = 0,  // INMP441不需要外部时钟
    ...
    ESP_LOGI(TAG, "I2S配置: SR=%lu, Bits=%d, Mode=MONO/LEFT, Format=Philips", 
             sample_rate, bits_per_chan);
...
esp_err_t bsp_get_feed_data(bool is_get_raw_channel, int16_t *buffer, int buffer_len)
{
    ...
    {
        ESP_LOGW(TAG, "预期读取%d字节，实际读取%d字节", buffer_len, bytes_read);
    }
...
esp_err_t bsp_audio_init(uint32_t sample_rate, int channel_format, int bits_per_chan)
{
    ...
    i2s_std_config_t std_cfg = {
        .clk_cfg = {
            ...
            .mclk_multiple = I2S_MCLK_MULTIPLE_256,
            .bclk_div = 8,  // BCLK分频系数（ESP-IDF 6.0+ 需要）
        },
    ...
    // 【关键修复】单声道播放时，必须设置slot_mask为左声道
    // MAX98357A在单声道模式下只接收左声道数据，不设置会导致没有声音
    if (channel_format == 1) {
        std_cfg.slot_cfg.slot_mask = I2S_STD_SLOT_LEFT;
        ESP_LOGI(TAG, "I2S配置: 单声道模式，slot_mask=LEFT");
    }
...
/**
 * @brief 重新启用 I2S 音频输出（用于流式播放）
 * 
 * 当 bsp_audio_stop() 被调用后，需要调用此函数重新启用 I2S 输出
 * 
 * @return esp_err_t 启用结果
 */
esp_err_t bsp_audio_resume(void)
{
    ...
    {
        ESP_LOGD(TAG, "I2S发送通道已经启用");
        return ESP_OK;
    }

    // 启用功放
    gpio_set_level(I2S_OUT_SD_PIN, 1);
    vTaskDelay(pdMS_TO_TICKS(10));
    ESP_LOGI(TAG, "MAX98357A功放已重新启用");

    // 启用I2S发送通道
    ret = i2s_channel_enable(tx_handle);
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "重新启用I2S发送通道失败: %s", esp_err_to_name(ret));
        return ret;
    }
    tx_channel_enabled = true;
    ESP_LOGI(TAG, "I2S音频输出已重新启用");

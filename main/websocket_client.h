...
public:
     ...
     * 
     * @param uri 服务器地址（如 ws://47.93.146.234:8000/ws/esp32）
     * @param auto_reconnect 是否自动重连（默认开启）
     * @param reconnect_interval_ms 重连间隔时间（默认5秒）
     * @param enable_keepalive 是否启用TCP保活（默认开启）
     * @param keepalive_idle 保活空闲时间（秒，默认5秒）
     * @param keepalive_interval 保活探测间隔（秒，默认5秒）
     */
...
private:
    ...
    bool enable_keepalive_;       // 是否启用TCP保活
    int keepalive_idle_;          // 保活空闲时间（秒）
    int keepalive_interval_;      // 保活探测间隔（秒）
    ...
    // 内部配置常量
    static constexpr int BUFFER_SIZE = 32768;               // 【增大】接收缓冲区32KB
    static constexpr int SEND_BUFFER_SIZE = 16384;          // 【增大】发送缓冲区16KB
    static constexpr int TASK_STACK_SIZE = 16384;           // 【增大】WebSocket任务栈大小
    static constexpr int RECONNECT_TASK_STACK_SIZE = 4096;  // 重连任务栈大小
    static constexpr int RECONNECT_TIMEOUT_MS = 2000;       // 首次重连时间缩短到2秒
};
